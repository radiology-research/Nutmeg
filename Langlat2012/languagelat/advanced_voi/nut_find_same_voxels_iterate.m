...
% that make sense?
% 8/11/2015 AMF updated to be compatible with nutmeg4+ analysis pipeline
% nut_timef_viewer removed, not needed
% Also changed d<=5 instead of d<=10

load(VOI_matfile);
%nut_results_viewer(s_beam_file);
load(s_beam_file);

voxelsize = beam.voxelsize; %commented in by L.B.N.H. (4/11/2016) 3:59pm
...
for timewin = 1:size(beam.s{1},2) %iterate over time windows, earliest first
    for freqbin = 1:size(beam.s{1},3) %iterate over frequency bins, lowest first
        ...
        for ii = 1:size(k,1)
            if d(ii) <= 5       %changed from 10 on 8/11/2015 AMF
                voxel_number(ii) = k(ii); %k(ii) is the index number of the VOI

            elseif d(ii) > 5    %changed from 10 on 8/11/2015 AMF
                voxel_number(ii) = 0;
            end
        end
