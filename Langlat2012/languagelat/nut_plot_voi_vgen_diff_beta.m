function [stimLIaverageVOI1, responseLIaverageVOI2, combinedLIaverage, LI1, LI2]=nut_plot_voi_vgen_diff_beta(subject_id)
%[stimLIaverageVOI1, responseLIaverageVOI2, combinedLIaverage, LI1, LI2]=nut_plot_voi_vgen_diff_beta(subject_id)
...
% 8/11/2015 AMF
% updated to reflect output of new tfbf analysis, will generate an s_beamtf
% file with either "response" or "stim" in the filename. This searches the
% directory for files matching that and will pick the FIRST one to get time
% window information. Must use new verbgen_plotdir script in shell first,
% then cd into voianalysis.
...
cd response
files=dir('s_beamtf_*response*CTFpseudoF.mat');
s_beam_response = files(1).name;
...
cd ../stim
files=dir('s_beamtf_*stim*CTFpseudoF.mat');
s_beam_stim = files(1).name;
...
responseLIaverageVOI2   = mean(LI2(9:11,1));
...
%     plot(boxtime(7:8),boxval(1:2),'y-')
    ...
    titlestring=['Laterality Index, ' subject_id ', ' num2str(beam_stim.beam.bands(ii,1)) '-' num2str(beam_stim.beam.bands(ii,2)) ' Hz;'...
        ' LI (stim) = ' num2str(stimLIaverageVOI1,3) ', LI (response) = ' num2str(responseLIaverageVOI2,3) ', LI (average) = ' num2str(combinedLIaverage,3) ];
    title(titlestring)
