function Fsummary=nut_voi_picturenaminglat(s_beam_file_nm4,subject_id,wadaresult)
...
nut_mni2languagevoi_pn(s_beam_file_nm4) %Create MNI-based VOIs using rivets.MNIdb
...
%Calculate average LI for VOI2, timepts -450, -350, -250 msec
format compact
index = find(timepts >= -450 & timepts <= -250);
fprintf('\nTime points to be used for average LI for VOI2,\n');
fprintf('response locked picture naming\n');
fprintf('should be -450, -350, -250 msec:\n\n')
%timepts(index)
fsummary_filename = (['fsummary_allLI_VOI2_' wadaresult 'wada'])
load(fsummary_filename);
fprintf('\nLI for time points %i, %i, and %i msec:\n', timepts(index(1)), timepts(index(2)), timepts (index(3)))
Fsummary.LI(index)
fprintf('\nMean LI over time points for VOI2:\n')
mean(Fsummary.LI(index))
