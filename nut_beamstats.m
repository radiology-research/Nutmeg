...
for nn=1:stats.numtests          
    ...
    if stats.dostat  % if stats requested
        ...
        cth1 = zeros(1,newtimepts,newfreqbins);
        ...
        for tt=1:newtimepts            % loop through time windows if requested.
            ...
            for ff=1:newfreqbins           % loop through frequency bands if requested.
                ...
                %------------------------------
                [T(:,tt,ff),nP(:,tt,ff),MaxT(:,:,ff,tt),MaxCluster(:,:,ff,tt),cth1(1,tt,ff)] = nut_snpm(X,Q,voxels3d,voxelsize,SP);
...
case 'anova_snpm'
    ...
    fprintf('\nPerforming SnPM ANOVA tests within subjects...\n')      
    ...
    if stats.numsubj<=12 || ~stats.isaproxperm          % exact method
        ...
        if stats.isaproxperm                 % pick random supsample of perms
            tmp=randperm(size(SP.PiCond,1));
            SP.PiCond=SP.PiCond(tmp(1:stats.numperm),:);
                % Note we may have missed iCond!  We catch this below.	
        end	
...
case 'anovabet_snpm'
    ...
    fprintf('\nPerforming SnPM ANOVA tests between subjects...\n')      

    rand('seed',sum(100*clock));        % Initialise random number generator
    nCond       = length(stats.condsel);    
    SP.sHCform  = 'spm_DesMtx(double(SP.PiCond(perm,:)),''-'',''Cond'')';
    SP.CONT     = spm_DesMtx(1:nCond, '+0')';
    SP.STAT     = 'F';
    SP.hPerms   = false;
    SP.numperm  = stats.numperm;
    ...
    iCond = stats.condnr;
    for i = 1:nCond
       iCond(iCond==stats.condsel(i)) = i;
    end
    GrpCnt = zeros(1,nCond);
    for i = 1:nCond
       GrpCnt(i) = sum(iCond==i);
    end
    hash = 1:stats.numsubj;
    hash = nCond.^(hash-1);
    true_hash = iCond * hash';

    if stats.numsubj<=10 || ~stats.isaproxperm          % exact method

        %-Generate all labellings of stats.numsubj scans as 1,2,3,...
        SP.PiCond=uint8([]);
        for i=0:stats.numsubj-1
          tmp = uint8([]);
          for j=1:nCond
            tmp = [tmp;uint8(j*ones(nCond^i,1)),SP.PiCond];
          end
          SP.PiCond = tmp;
        end
        ...
        for i=1:(nCond-1)
          SP.PiCond=SP.PiCond(sum(SP.PiCond'==i)==GrpCnt(i),:);
        end
        ...
        for i=2:stats.numperm
          tmp=SP.PiCond(i-1,randperm(stats.numsubj));
          while any(abs(SP.PiCond(1:(i-1),:)*hash' - tmp*hash')/(tmp*hash') < sqrt(eps))
             tmp=SP.PiCond(i-1,randperm(stats.numsubj));
          end
          SP.PiCond(i,:)=tmp;
        end      
        SP.PiCond = uint8(SP.PiCond);
    end
    ...
    if ~all(all(double(SP.PiCond)*ones(stats.numsubj,1)==row_total))
        error('Invalid SP.PiCond computed!')
    end

    %-Find (maybe) iCond in SP.PiCond, move iCond to 1st; 
    %-----------------------------------------------------------------------
    perm_hash = double(SP.PiCond) * hash';
    perm = find(abs(true_hash-perm_hash)/true_hash < sqrt(eps));

    if length(perm) > 1
        perm = find(all((meshgrid(iCond,1:size(SP.PiCond(perm,:),1))==SP.PiCond(perm,:))'));
        if (perm~=1)
        SP.PiCond(perm,:)=[];
        SP.PiCond=[iCond;SP.PiCond];
        end
    ...
    elseif isempty(perm) && (stats.numsubj<=10) && stats.isaproxperm
        % If ~bAproxTst, we won't miss iCond;
        % If (stats.numsubj>10)& bAproxTst, we use random method, iCond is guaranteed to be there.
        % So the only way in which we miss iCond is: (stats.numsubj<=10) & bAproxTst.
        % Special case where we missed iCond; order of perms is random 
        % so we can just replace first perm.
        SP.PiCond(1,:) = iCond;
        perm = 1;
    else    
        error(['Bad SP.PiCond (' num2str(perm) ')'])
    end                       
    clear tmp a d rand_tmp perm
