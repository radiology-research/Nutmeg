...
% 
% 
% function [a,b,lam,nu,alp,bet,xbar,ubar,ubar0]=sefa0(y,y0,nl,nm,nem,a_init,b_init,lam_init,nu_init);
% 
% % vb-em algorithm for inferring the sefa analysis model   
% % y = a*x + b*u + v ,   y0 = b*u0 + v0,   
% % u~N(0,nu) ,   x,u0~N(0,I) ,   v,v0~N(0,lam)
% % learn a,b,lam,nu
% %
% % y(nk,nt) = data
% % a(nk,nl) = mixing matrix
...
%    ldpsi=sum(log(svd(psi)));
%    like0=.5*nt0*(ldlam-ldgam0)-.5*sum(sum(y0.*(lam*y0)))+.5*sum(sum(ubar0.*(gam0*ubar0)));
%    like(iem)=.5*nt*(ldlam-ldgam)-.5*sum(sum(y.*(lam*y)))+.5*sum(sum(xubar.*(gam*xubar)))+.5*nk*(ldalpbet+ldpsi)+like0+.5*nt*sum(log(nu));
% 
%    subplot(3,3,3);plot((1:iem)',like(1:iem)/(nt+nt0));title('sefa0 like');
%    subplot(3,3,6);plot([mean(ab.^2,1)' 1./diag(alpbet)]);title('1/alp,bet')
%    subplot(3,3,9);plot(1./diag(lam));title('1/lam');
%    subplot(3,3,8);plot(1./nu);title('1/nu');
%    drawnow;
% 
%    rxuxu=xubar*xubar'+nt*igam;
%    ruu0=ubar0*ubar0'+nt0*igam0;
...
%    alpbet=diag(nk./diag(alapsi));
% 
%    ruu=ubar*ubar'+nt*igam(nl+1:nlm,nl+1:nlm);
%    nu=nt./diag(ruu);
% end
% 
% a=ab(:,1:nl);
