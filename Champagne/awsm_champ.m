...
function [gamma,x,w]=awsm_champ(y,f,sigu,nem,nd,vcs,plot_on);

if vcs==2 && nd>1
    [gamma x w l v]=champ_mat(y,f,sigu,nem,nd,plot_on);
else
    [gamma x w l v]=champ_vec(y,f,sigu,nem,nd,vcs,plot_on);
end
...
function [gamma,x,w,like,vvec]=champ_vec(y,f,sigu,nem,nd,vcs,plot_on);
...
like=zeros(nem,1);
for iem=1:nem;
    fprintf(repmat('\b', 1, str_length-1));
    % Create the progress string
    str = sprintf('Progress: %3.1f%%', iem / nem * 100);
    % Update the length of the printed string
...
%        v0(ff)=sqrt(x20(ff)./z0(ff));
        ...
        vvec=zeros(size(x2));
        ff=find(z>0);
        vvec(ff)=sqrt(max(x2(ff)./z(ff),0)); % CN 10/2012
...
function [gamma,x,w,like,vvec]=champ_mat(y,f,sigu,nem,nd,plot_on);
